...
def generate_questions(question, answer, translate):
    ...
    res = syntetitc_data_generator.invoke(
        {"example": question + " Відповідь: " + answer}
    )
    generated_questions = ""
    for i, tool_call in enumerate(res.additional_kwargs["tool_calls"]):
        generated_question = json.loads(tool_call["function"]["arguments"])[
            "paraphrased_query"
        ]
...
def add_sample_for_cross_encoder(df, bucket_name="chatbot-rag"):
    ...
    try:
        ...
        logging.error(
            f"Some issue occured while fetching samples from Minio!, Error: {e}"
        )
...
def remove_file_from_minio(bucket_name="chatbot-rag", file_name="context.csv"):
    # Initialize Minio client
    client = minio.Minio(
        minio_url, access_key=minio_login, secret_key=minio_password, secure=False
    )
    try:
        ...
        return f"Some issue occured while removing file from Minio!, Error: {e}"


if __name__ == "__main__":
    df = pd.DataFrame(
        [
            {"query": "ss", "positive": "sss", "negative": "s"},
            {"query": "ssss", "positive": "sss", "negative": "s"},
        ]
    )
