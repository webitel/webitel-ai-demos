...
evaluator = Evaluator(user_metadata={"ContractNumber": "№12345",
                "DueDate": "2024-05-13",
...
def process_history(chat_history):
    ...
    for i,message in enumerate(chat_history):
        ...
        try:
            message_2 = f'{chat_history[2*i+1]["sender"]}:\n' +   chat_history[2*i+1]['message']
        except:
            message_2 = ""
...
assert len(model_A_history) == len(model_B_history), "The number of chat histories should be the same for both models."
...
def save_choice(choice):
    ...
    pd.DataFrame(results_of_evalutaion.items(), columns=["Chat History Index", "Choice"]).to_csv("evaluation_results.csv", index=False)
    ...
    index_of_chat_history = random.choice(list(set(all_indicies).difference(used_indicies)))
...
with gr.Blocks() as demo:
    with gr.Tab("A/B Evaluation"):
        ...
        btn_left_better.click(lambda: save_choice("A Better"), inputs=[], outputs=[ chatbot_a, chatbot_b])
        btn_right_better.click(lambda: save_choice("B Better"), inputs=[], outputs=[ chatbot_a, chatbot_b])
        btn_both_bad.click(lambda: save_choice("Both Bad"), inputs=[], outputs=[ chatbot_a, chatbot_b])
        btn_both_good.click(lambda: save_choice("Both Good"), inputs=[], outputs=[ chatbot_a, chatbot_b])
    ...
    with gr.Tab("Automated Evaluation"):
        ...
        eval_btn.click(evaluator.evaluate, outputs=[score_plot,exact_match_plot,embedding_plot])
        u = gr.UploadButton("Upload JSON with conversations",file_count='single', type = 'binary')
