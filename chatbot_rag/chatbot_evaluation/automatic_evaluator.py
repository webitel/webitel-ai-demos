...
"""
        ...
        self.evaluator = load_evaluator("labeled_score_string",criteria=self.accuracy_criteria ,llm=ChatOpenAI(model="gpt-4o",temperature=0))
        self.embedding_evaluator = load_evaluator("embedding_distance", embeddings=Embedding())
    ...
    def generate_answer(self, chat_history):
        ...
        response = self.stub.Answer(chatbot_pb2.MessageRequest(user_metadata=self.user_metadata, categories=['all'], messages=chat_history))
    ...
    def evaluate_answer(self,user_question,generated_answer,reference_answer ):
        ...
        print(eval_result)
        eval_result['exact_match'] = self.exact_match_evaluator.evaluate_strings(prediction=generated_answer,reference=reference_answer)
        eval_result['embedding_distance'] = self.embedding_evaluator.evaluate_strings(prediction=generated_answer,reference=reference_answer)
        return eval_result
    ...
    def evaluate(self):
        ...
        for chat in tqdm(self.evaluation_data):
            ...
            for message in chat:
                if message['operator'] and chat_history:
                    ...
                    #TODO add eval here
                    evaluation_result = self.evaluate_answer(user_question =chat_history[-1].message,generated_answer=text, reference_answer=message['text'])
                    
                    results.append({'user_question':chat_history[-1].message,'generated_answer':text,'reference_answer':message['text'],'score':evaluation_result['score'], 'reasoning':evaluation_result['reasoning'], 'exact_match':evaluation_result['exact_match'], 'embedding_distance':evaluation_result['embedding_distance']})
                ...
                proto_message = chatbot_pb2.Message(sender='ai' if message['operator'] else 'human',message=text)
        ...
        pie_chart_scores = create_bar_plot(plot_data, title='Distribution of Scores',x_title="Score")
        pie_chart_exact_match = create_bar_plot(exact_match_data, title='Distribution of Scores',x_title="Exact Match")
        chart_embedding_scores = alt.Chart(pd.DataFrame({'Embedding Distance': embedding_distance_data})).mark_boxplot().encode(
            y=alt.Y('Embedding Distance:Q', title='Embedding Distance')
        ).properties(
            title='Box Plot of Embedding Distances'
        )
...
def create_bar_plot(data, title, x_title):
    ...
    bar_chart = alt.Chart(df).transform_aggregate(
        count='count()', groupby=['Value']
    ).mark_bar().encode(
...
if __name__ == "__main__":
    ...
    res = embedding_evaluator.evaluate_strings(prediction= "Привіт",reference = "Добрий день")
    print(res)
    ...
    res = exact_match_evaluator.evaluate_strings(prediction= "Привіт",reference = "Добрий день")
    print(res)